...
canvas.pack()
sh.geometry("800x600")         #设置窗口尺寸
l1=tk.Label(sh,text="deep")    #标签
l11=tk.Entry(textvariable=sha) #创建文本框
l1.pack()
l11.pack()
l2=tk.Label(sh,text="H") #标签
l22=tk.Entry(textvariable=shb) #创建文本框
l2.pack()
l22.pack()
l3=tk.Label(sh,text="A") #标签
l33=tk.Entry(textvariable=shc) #创建文本框
l3.pack()
l33.pack()
l4=tk.Label(sh,text="Lref(默认同水深)") #标签
l44=tk.Entry(textvariable=shd) #创建文本框
l4.pack()
l44.pack()
l5=tk.Label(sh,text="Uref") #标签
l55=tk.Entry(textvariable=she) #创建文本框
l5.pack()
l55.pack()
 #command绑定获取文本框内容方法
canvas.create_window(420, 260, width=100, height=20,
                                       window=l55)
canvas.create_window(320, 260, width=100, height=20,
                                       window=l5)
canvas.create_window(420, 220, width=100, height=20,
                                       window=l44)
canvas.create_window(320, 220, width=100, height=20,
                                       window=l4)
canvas.create_window(420, 180, width=100, height=20,
                                       window=l33)
canvas.create_window(320, 180, width=100, height=20,
                                       window=l3)
canvas.create_window(420, 140, width=100, height=20,
                                       window=l22)
canvas.create_window(320, 140, width=100, height=20,
                                       window=l2) 
canvas.create_window(420, 100, width=100, height=20,
                                       window=l11)
canvas.create_window(320, 100, width=100, height=20,
                                       window=l1)   
...
def ss1():  
    ...
    gg=replace(53,cff)
    Lref=replace(11,Lref)
    Uref=replace(12,Uref)
    hh=replace(117,cff)
    d='static double cf ='+str(cff)
    ...
    gre=g/Gref
    d='-'+str(gre)
    bb=re.sub(r"-+(\d+)",d,lines[63])
    lines[63]=bb
    y22=y2/(ss+y1)
    d='*'+str(y22)
    bb=re.sub(r"\*+(\d+\.\d+)",d,lines[63])
    lines[63]=bb
    d='('+str(y22)
    bb=re.sub(r"\(+(\d+\.\d+)",d,lines[64])
    lines[64]=bb
    d='nm_2*dhp/4*'+str(ss)
    bb=re.sub(r"nm_2\*dhp\/4\*(\d+)",d,lines[118])
    lines[118]=bb
    d='-atan('+str(-c1)
    bb=re.sub(r"-atan\((\d+)",d,lines[118])
    lines[118]=bb
    d=')-'+str(d1)
    bb=re.sub(r"\)-(\d+\.\d+)",d,lines[118])
    lines[118]=bb
    d='))-'+str(y1)
    bb=re.sub(r"\)\)-(\d+\.\d+)",d,lines[118])
    lines[118]=bb
